"""
Simple log handler module, used for web application log capture
"""
import threading
...
# Register custom log handler, store logs by session ID
class SessionLogHandler:
    ...
    def __call__(self, record):
        ...
        # Pass the record, continue processing the chain
        return True


class SimpleLogCapture:
    """Simple log capture, provides similar logger interface"""
...
def capture_session_logs(session_id: str):
    """
    Context manager for capturing session logs
    Returns a SimpleLogCapture instance, not the log list directly
    """
    # Create session log storage
    with _lock:
    ...
    try:
        # Return log capture instead of log list
        yield log_capture
    finally:
