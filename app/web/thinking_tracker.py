...
class ThinkingStep:
    ...
    def __init__(
        ...
        self.details = details  # For storing communication content or detailed information
...
class ThinkingTracker:
    ...
    # Class variable, storing thinking steps for all sessions
    ...
    _ws_send_callbacks: Dict[str, Any] = {}  # Stores WebSocket send callback functions
    ...
    def start_tracking(cls, session_id: str) -> None:
        """Start tracking the thinking process of a session"""
        with cls._lock:
    ...
    def add_communication(cls, session_id: str, direction: str, content: str) -> None:
        ...
        Args:
            ...
            direction: Communication direction, such as "Send to LLM", "Receive from LLM"
            ...
            if session_id in cls._session_progress:
                ...
                if progress["total_steps"] > 0:
                    progress["percentage"] = min(
                        ...
                        99,  # Maximum 99%, only reaches 100% when completed
                    )
    ...
    def clear_session(cls, session_id: str) -> None:
        """Clear specified session's records"""
        with cls._lock:
            if session_id in cls._session_steps:
    ...
    def add_log_entry(cls, session_id: str, entry: Dict) -> None:
        ...
        with cls._lock:
            ...
            # Automatically add corresponding thinking steps based on log level
            msg = entry.get("message", "")
            if entry.get("level") == "INFO":
                # Generate more meaningful thinking steps for specific log content
                if "Start executing" in msg:
                    ...
                    # Attempt to extract step information
    ...
    def _update_progress_from_log(cls, session_id: str, message: str):
        """Update progress information from log message"""
        import re

        # Attempt to extract step information from log message
...
CODING_STEPS = [
    ...
    "Write main function modules",
    "Implement boundary cases and error handling",
    "Test and debug code",
...
WRITING_STEPS = [
    "Gather writing topic-related materials",
...
def generate_thinking_steps(
    ...
    # If description is provided, add more specific steps
    if task_description:
        specific_steps = [
            f"Research {task_description} related information",
            f"Analyze {task_description} key points",
            f"Organize {task_description} solutions",
        ]
    ...
    for step in steps:
        ...
        if show_communication:
            ...
            ThinkingTracker.add_communication(session_id, "Send to LLM", f"Please help me {step}...")

            # Simulate receiving response from LLM
            ThinkingTracker.add_communication(
                session_id, "Receive from LLM", f"I have completed {step}. Here is the relevant result: [Detailed information]"
            )

    # Add conclusion
    ThinkingTracker.add_conclusion(session_id, "Task processing completed! Final answer generated.")
