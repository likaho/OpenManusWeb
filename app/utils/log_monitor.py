...
class LogFileMonitor:
    def __init__(self, job_id=None, log_dir="logs"):
        # Use environment variable task ID first
        self.job_id = job_id or os.environ.get("OPENMANUS_TASK_ID")
        self.log_dir = log_dir

        # Use environment variable log file path first
    ...
    def start_monitoring(self):
        ...
        # If log file exists, read existing content
    ...
    def get_new_entries_since(self, timestamp):
        """Get new log entries since the specified timestamp"""
        ...
        for i in range(len(self.log_entries) - 1, -1, -1):
            # Here we simplify the process, assuming all new entries are added consecutively
            # Actual implementation may require adding timestamps to log entries
            if i >= len(self.log_entries) - 10:  # Return at most the last 10 entries
